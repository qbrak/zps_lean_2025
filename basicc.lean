  ...
  have x_lim_in_balls : ∀ (n : ℕ), x_lim ∈ Metric.closedBall (x n) (r n) := by
    ...
    have x_k_in_ball_x_n : ∀ k ≥ n, x k ∈ Metric.closedBall (x n) (r n) := by
      ...
      | succ m mgt_n hm_ind =>
        ...
        have ball_m_subset_ball_n : Metric.closedBall (x m) (r m) ⊆ Metric.closedBall (x n) (r n) := by
          ...
          | succ i hi_ge_n hi_subset =>
            ...
            have step_nested := (h_nested_balls.left.left i).left
        ...
        have x_next_in_ball_m : x (m + 1) ∈ Metric.closedBall (x m) (r m) := by
          ...
          have step := (h_nested_balls.left.left m).left
          -- x_{m+1} jest środkiem B_{m+1}, więc w niej leży
          have center_in_self : x (m + 1) ∈ Metric.closedBall (x (m + 1)) (r (m + 1)) :=
            Metric.mem_closedBall_self (le_of_lt (h_nested_balls.right (m + 1)).left)
        ...
        exact ball_m_subset_ball_n x_next_in_ball_m
